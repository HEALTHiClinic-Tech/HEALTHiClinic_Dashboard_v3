/**
 * Script to populate Dr. Carlos Tahuil-Ochoa's weekly appointment data
 *
 * Usage: node scripts/populate-carlos-tahuil-ochoa-appointments.js
 *
...
// Dr. Carlos Tahuil-Ochoa's appointment data extracted from screenshots
const appointmentData = [
  { weekStart: '2025-10-27', year: 2025, weekNumber: 44, appointments: 10 },  // Oct 27 - Nov 2
  { weekStart: '2025-11-03', year: 2025, weekNumber: 45, appointments: 13 },  // Nov 3 - Nov 9
  { weekStart: '2025-11-10', year: 2025, weekNumber: 46, appointments: 22 },  // Nov 10 - Nov 16
  { weekStart: '2025-11-17', year: 2025, weekNumber: 47, appointments: 21 },  // Nov 17 - Nov 23
  { weekStart: '2025-11-24', year: 2025, weekNumber: 48, appointments: 11 },  // Nov 24 - Nov 30
  { weekStart: '2025-12-01', year: 2025, weekNumber: 49, appointments: 15 },  // Dec 1 - Dec 7
  { weekStart: '2025-12-08', year: 2025, weekNumber: 50, appointments: 11 },  // Dec 8 - Dec 14
  { weekStart: '2025-12-15', year: 2025, weekNumber: 51, appointments: 16 },  // Dec 15 - Dec 21
...
async function populateAppointments() {
  console.log('🔍 Looking for Dr. Carlos Tahuil-Ochoa in the database...\n');

  // Step 1: Find Dr. Carlos Tahuil-Ochoa's ID
  const { data: doctors, error: doctorError } = await supabase
    .from('doctors')
    .select('*')
    .eq('first_name', 'Carlos')
    .eq('last_name', 'Tahuil-Ochoa');
  ...
  if (!doctors || doctors.length === 0) {
    console.error('❌ Dr. Carlos Tahuil-Ochoa not found in the database.');
    console.log('Please make sure the doctor exists in the doctors table.');
    process.exit(1);
  }

  const doctor = doctors[0];
  console.log(`✅ Found Dr. Carlos Tahuil-Ochoa!`);
  ...
  console.log('═══════════════════════════════════════════════════════════');
  console.log('📋 SUMMARY - Dr. Carlos Tahuil-Ochoa Appointments');
  console.log('═══════════════════════════════════════════════════════════');
  ...
  appointmentData.forEach(data => {
    ...
    if (data.appointments >= target) targetMetWeeks++;
  ...
  console.log(`🌟 Weeks Target Met: ${targetMetWeeks}/${appointmentData.length}`);
  console.log('═══════════════════════════════════════════════════════════');
